...
function createBuilder() {
  ...
  // add import
  lines = [
    `import { ${pascalName}Memo as ${pascalName} } from '../../heroes/${pascalName}';`,
    ...lines,
  ];

  // add to render loop
  const jsx = `\n{/* ${name} */}{hero._type === '${schemaName}' && <${pascalName} {...hero} />}`;
  if (lines.find((x) => x.indexOf('<></>') > -1)) {
    lines[lines.findIndex((x) => x.indexOf('<></>') > -1)] = `return <>${jsx}</>`;
  } else {
    lines = addLine(jsx, lines, '</>', 0, '<>');
  }
