...
export const Video = ({
  ...
  const [responsiveSrc, setResponsiveSrc] = useState<string | null>(videoId);
  ...
  useEffect(() => {
    ...
    const plyr = new PlyrJS(videoRef.current || videoDivRef.current, options);
    plyr.muted = autoPlay; // autoplay won't work on chrome without this, seems a bug that it's not accepting the muted option
    ...
    if (window.matchMedia('(prefers-reduced-motion: reduce)').matches) plyr.pause();
      ...
      {lazyLoaded && (
        <AnimateInView>
          ...
          {(provider === 'static' ||
            ...
            provider === 'mux' ||
            provider === 'cloudinary') && (
            <video playsInline ref={videoRef} muted={autoPlay}>
              <source
                src={responsiveSrc}
                type={provider === 'mux' ? 'application/x-mpegURL' : 'video/mp4'}
              />
            </video>
          )}
          ...
          {frameless && (
            ...
            >
              <Icon
                name={videoPlaybackState === 'playing' ? 'PauseIcon' : 'PlayIcon'}
              />
            </button>
          )}
        </AnimateInView>
